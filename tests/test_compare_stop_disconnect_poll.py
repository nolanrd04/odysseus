...
def test_compare_mode_branch_skips_agent_runs_in_source():
    ...
    # Match the call, not its full argument list — start() also takes
    # requested_at= now (so a Stop during a long pre-flight isn't lost), and
    # pinning the exact text made this fail on an unrelated signature change.
