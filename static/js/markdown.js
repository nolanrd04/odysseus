...
export function mdToHtml(src, opts) {
  ...
  if (window.katex) {
    ...
    // currency doesn't render as math ("$5 to $10", "$21,000 ... $24,000"): the
    // opening $ must be immediately followed by a non-space, the closing $ must
    // be immediately preceded by a non-space and not followed by a digit.
    s = s.replace(/(?<![\$\d])\$(?!\$)(?=\S)([^\$\n]+?)(?<=\S)\$(?!\$|\d)/g, (match, math) => {
      try {
        const raw = math.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
        const placeholder = `___MATH_BLOCK_${mathBlocks.length}___`;
